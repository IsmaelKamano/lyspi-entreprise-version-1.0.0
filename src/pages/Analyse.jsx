...
const Analyse = () => {
  ...
  const [filters, setFilters] = useState({
    poste: 'Tous',
    typeOffre: '', // Initialisé à vide pour forcer la sélection du premier typeOffre
  });
  ...
  const fetchStudentData = _.debounce(async () => {
    ...
    try {
      console.log('Récupération des données des étudiants avec paramètres', {
        poste: filters.poste,
  ...
  const fetchTypeOffres = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setTypeOffres(types);
        // Sélectionne le premier typeOffre par défaut si aucun filtre n'est défini
        if (types.length > 0 && !filters.typeOffre) {
          setFilters((prev) => ({ ...prev, typeOffre: types[0].id.toString() }));
        }
        console.log('Types d\'offres récupérés', { count: types.length });
      }
    } catch (err) {
    ...
    if (!loading && statsChartRef.current && !error) {
      ...
      try {
        ...
        statsChartInstanceRef.current = new Chart(ctx, {
          type: 'pie',
          data: {
            labels: ['Candidats acceptés', 'Embauches en attente', 'Embauchés'],
            datasets: [{
              label: 'Statistiques',
              data: [
          ...
          options: {
            ...
            plugins: {
              legend: { position: 'top' },
              title: { display: true, text: `Statistiques des postulations acceptées (Entreprise: ${entreprise.nom_entreprise || 'N/A'})` },
            },
          },
        });
      } catch (err) {
  ...
  useEffect(() => {
    if (!loading && typeOffreChartRef.current && byTypeOffre.length > 0 && !error) {
      ...
      try {
        ...
        const data = byTypeOffre.map(row => row.total_students || 0);
        ...
        typeOffreChartInstanceRef.current = new Chart(ctx, {
          ...
          data: {
            labels,
            datasets: [{
              label: 'Candidats acceptés par type d\'offre',
              data,
          ...
          options: {
            ...
            scales: {
              y: { beginAtZero: true, title: { display: true, text: 'Nombre' } },
              x: { title: { display: true, text: 'Type d\'offre' } },
            },
          },
        });
      } catch (err) {
  ...
  useEffect(() => {
    if (!loading && faculteChartRef.current && byFaculte.length > 0 && !error) {
      ...
      try {
        ...
        const data = byFaculte.map(row => row.total_students || 0);
        ...
        faculteChartInstanceRef.current = new Chart(ctx, {
          ...
          data: {
            labels,
            datasets: [{
              label: 'Candidats acceptés par faculté',
              data,
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Filtres: Poste - ${filters.poste} | Type d'offre - ${filters.typeOffre ? typeOffres.find(t => t.id === parseInt(filters.typeOffre))?.nom || 'N/A' : 'Premier disponible'}`, 50, 46);

    // Stats Table
    autoTable(doc, {
      startY: 54,
      head: [['Poste', 'Type d\'offre', 'Candidats acceptés', 'Embauches en attente', 'Embauchés']],
      body: [[
        filters.poste,
        filters.typeOffre ? typeOffres.find(t => t.id === parseInt(filters.typeOffre))?.nom || 'N/A' : 'Premier disponible',
        stats.total_students_applied || 0,
        stats.pending_hires || 0,
        stats.hired || 0,
      ]],
    ...
    doc.save(`rapport-analyse-postulations-ent-${entreprise.nom_entreprise || 'inconnu'}.pdf`);
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `rapport-analyse-postulations-ent-${entreprise.nom_entreprise || 'inconnu'}.csv`);
  ...
  const handleFilterChange = (key, value) => {
    ...
    setFilters(prev => ({
      ...prev,
      [key]: value,
      ...(key === 'typeOffre' ? { poste: 'Tous' } : {}), // Reset poste si typeOffre change
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-white p-6 flex flex-col">
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white shadow-xl rounded-xl p-6 transform hover:scale-105 transition duration-300">
                ...
                <p className="text-lg text-gray-600"><span className="font-semibold">Candidats acceptés :</span> {stats.total_students_applied || 0}</p>
              ...
              {/* Graphique à barres des types d'offres */}
              <div className="bg-white shadow-xl rounded-xl p-6 transform hover:scale-105 transition duration-300">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">Candidats acceptés par type d'offre</h2>
                <div className="w-full h-80">
                  <canvas ref={typeOffreChartRef} />
                </div>
              </div>
